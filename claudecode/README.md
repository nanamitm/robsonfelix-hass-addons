...
- **Usage Sensors**: Session, weekly, Sonnet, reset-time, and extra-usage sensors through MQTT Discovery
...
| `usage_sensors` | Publish Claude subscription usage as MQTT sensors | true |
| `usage_poll_seconds` | Usage polling interval in seconds | 300 |

### Usage Sensors

The bridge reads Claude Code's existing OAuth credentials without modifying or
refreshing them. An MQTT broker such as the Mosquitto add-on is required. Reset
timestamps are normalized to the nearest minute so Anthropic's occasional
one-second timestamp jitter does not create false state changes or automation
triggers.

The usage API adapter is based on the MIT-licensed
[`hass-claude-usage`](https://github.com/trickv/hass-claude-usage) project.
